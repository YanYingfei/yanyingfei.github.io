...
定义第$i$个composed degree $d_i = deg(Q_i)\cdot (2^h-1) - |\mathsf{H}_i|$。
...
因此等号右侧可以总结成一个关于变量$X$的多项式$R^{\vec{r}}_i(X)$，该多项式的degree小于$d_{max}$。
...
以及 $\mathsf{aux}_{\mathsf{FRI}}=(\vec{t},s), 2^{-R}=2^h/2^{k'}$。
假设存在证明者$\mathcal{P}^*$，
