...
$C_i = (Q_i, \mathsf{H}_i)$组成。
	...
	- 用$|Cset|$表示the arithmetic complexity of the constraints，定义为$$\begin{equation}|Cset| \sum^s_{i=1} (|Q_i|+ |\mathsf{H}_i|)\end{equation}$$，其中，$|Q_i|$是计算多项式$Q_i$的算术电路，$|\mathsf{H}_i|$ 是vanishing polynomial $\mathsf{Z}_{\mathsf{H}_i}$的算术复杂度。
...
$Q \circ \vec{P}$ vanishes on $\mathsf{H}_i$。
...
验证 proximity to 函数 
$g_{(\vec{r},q,answer, r^F)}:\mathsf{D} \to \mathbb{K}$ 的
RS码 $\mathsf{RS}[\mathbb{K},\mathsf{D}, \rho]$。
其中，
