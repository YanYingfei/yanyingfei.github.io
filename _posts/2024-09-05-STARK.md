...
验证 proximity to 
函数 $$\beging{equation} g_{(\vec{r},q,answer, r^F)}:\mathsf{D} \to \mathbb{K} \end{equation}$$ 的
RS码 $$\beging{equation}\mathsf{RS}[\mathbb{K},\mathsf{D}, \rho] \end{equation}$$。
...
2. 存在一个证明者$$\begin{equation}\mathcal{P}^*(\mathsf{A}, \mathsf{aux})$ 在Step1 oracle发送$f=(f_1, ..., f_w) \end{equation}$$， 
且$\mathcal{V}$与$\mathcal{P}$交互时，
$\mathcal{V}$接受的概率
大于 \begin{equation} \epsilon_{total} = \frac{\ell}{|\mathbb{K}|} + \frac{(d_{max}+2^h + a) \cdot \ell}{|\mathbb{K}|-a|\mathsf{D}| + |\mathsf{H}_0|} + \epsilon_{\mathsf{FRI}}(q,n,\rho, m,s,\vec{t}), \end{equation} 
其中整数$m \geq 2, \ell=m / \rho$.
