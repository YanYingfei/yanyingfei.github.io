...
$\epsilon_{total} = \frac{\ell}{|\mathbb{K}|} + \frac{(d_{max}+2^h + a) \cdot \ell}{|\mathbb{K}|-a|\mathsf{D}| + |\mathsf{H}_0|} + \epsilon_{\mathsf{FRI}}(q,n,\rho, m,s,\vec{t}),$ 
...
假设存在证明者$\mathcal{P}^*$，满足

$$\begin{equation}\Pr[\langle \mathcal{V}(\mathsf{A, aux}, ), \mathcal{P}(\mathsf{A, aux})\rangle = accept] \geq 2\cdot \epsilon_{total},\end{equation}\$$
