...
- $$\begin{equation} \sigma \leq|S| / |\mathsf{D}| \end{equation}$$，
- 存在$v_1, ..., v_k \in V$，使得，
...
给定$$\begin{equation} \mathsf{A} = (\mathbb{F}, w, h, d, s, g, I, Cset), \mathsf{aux} = ( \mathbb{K}, e, \mathsf{D}, k',\mathsf{aux}_{\mathsf{FRI}}) \end{equation}$$ 以及 $$\begin{equation} \mathsf{aux}_{\mathsf{FRI}}=(\vec{t},s), 2^{-R}=2^h/2^{k'} \end{equation}$$。
