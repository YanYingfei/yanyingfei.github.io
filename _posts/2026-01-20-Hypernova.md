...
A multivariate polynomial $g$ over a finite field $$\mathbb{F}$$ is called low-degree polynomial
if the degree of $g$ in each variable is exponentially smaller than $$|\mathbb{F}|$$.

    - Low-degree $$\approx$$ 'poly' degree: for each variable, 
    the degree must be "exponentially smaller" than $$|\mathbb{F}|$$. 
    This typically means the degree is polylogarithmic in the field size, something like $$\mathcal{O}(\log^k |\mathbb{F}|)$$ for some constant $k$.
...
1. *CCS*. An $\mathcal{R}_{CCS}$ structure $\mathcal{S}$ consists of:
    ...
    - An $$\mathcal{R}_{CCS}$$ instance (structure-context tuple 
    $$(\mathcal{S}, x)$$) and an $\mathcal{R}_{CCS}$ witness 
    $$w \in \mathbb{F}^{n-\ell-1}$$ satisfy
...
2. CCS in MLE Polynomials
    ...
    - each line of $${\sum}_{i\in[q]} c_i~ \circ_{j \in S_i}~ (M_j \cdot \vec{z})$$
    (CCS equation) becomes $$\sum_{i=1}^q c_i (\Pi_{j}(\sum_{y\in \{0,1\}^{s'}} \tilde{M}_i(x,y) \cdot \tilde{z}(y)))$$ (MLE polynomial).
...
3. $\mathcal{V} \leftrightarrow \mathcal{P}$: 
Run the sum-check protocol $$c \leftarrow \langle \mathcal{P}, \mathcal{V}(r'_x) \rangle(g, s, d+1, \sum_{j \in [t]} \gamma^j \cdot v_j)$$ for the polynomial $g(x)$, where:
