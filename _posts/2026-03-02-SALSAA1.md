---
layout: post
title: "Norm Check in SALSAA folding"
categories: snark
author: Yingfei
meta: "Springfield"
---

SALSAA is a lattice-based folding scheme. This time we will go over how SALSAA achieves their norm check based on canonical embedding and Euclidean norm.

## Norm and inner-product

1. [2-Norm]. For a vector $\vec{v} = (v_1, v_2,...,v_n) \in \mathbb{R}^n$, 
its Euclidean norm(2-norm, $\ell_2$ norm) is $$||\vec{v}|| = \sqrt{v_1^2 + ... + v_n^2}.$$

So 
$$||\vec{v}||^2 = v_1^2 + ... + v_n^2 = \langle\vec{v}, \vec{v} \rangle$$

If
$$\vec{v} \in \mathbb{C}^n, ||\vec{v}||^2 = \sum_{i=1}^{n} v_i \bar{v}_i = \langle \vec{v}, \vec{\bar{v}}\rangle$$
...
the canonical embedding $\sigma : K \to \mathbb{C}^{d}$. 
Specifically, for a given $$\mathbb{Z}$$-basis $\mathbf{b} = (b_i)_{i \in [d]}$ of 
$$\mathcal{R}$$ and an element 
$$x = \sum_{i \in [d]} x_i b_i \in \mathcal{R}$$, 
we write $$\mathrm{cf}_{\mathbf{b}}(x) := (x_i)_{i \in [d]}$$ and $$\sigma(x) := (\sigma_j(x))_{j \in [d]}$$, 
where $$\sigma_j \in \text{Gal}(K/\mathbb{Q})$$. 
...
We extend the notation of $\sigma$ naturally to vectors, i.e. 
if $$\mathbf{x} = (x_i)_{i \in [m]} \in \mathcal{R}^m$$, then $$\sigma(\mathbf{x}) := (\sigma(x_i))_{i \in [m]}$$ is defined as concatenations.
...
Note that $$\|\mathbf{x}\|_{\sigma,2}^2 = \text{Trace}(\langle \mathbf{x}, \overline{\mathbf{x}} \rangle)$$, where $\overline{\cdot}$ denotes the complex conjugate. 
For a Galois extension $$\mathcal{M}/\mathcal{L}$$, the field trace can be computed as $$\text{Trace}_{\mathcal{M}/\mathcal{L}} : K \to \mathcal{L}$$, $$\text{Trace}_{\mathcal{M}/\mathcal{L}}(x) := \sum_{\sigma_j \in \text{Gal}(K/\mathcal{L})} \sigma_j(x)$$. When $$\mathcal{L} = \mathbb{Q}$$, we write $$\text{Trace} = \text{Trace}_{\mathcal{M}/\mathbb{Q}}$$.
...
When $q$ has multiplicative order $e$ modulo $f$ then $$\mathcal{R}_q$$ splits into $d/e$ finite fields, i.e. $$\mathcal{R}_q \cong (\mathbb{F}_{q^e})^{d/e}$$. 
We denote this isomorphism as $$\text{CRT} : \mathcal{R}_q \to (\mathbb{F}_{q^e})^{d/e}$$ and its inverse as $$\text{CRT}^{-1} : (\mathbb{F}_{q^e})^{d/e} \to \mathcal{R}_q$$. 
We naturally generalise this notation to vectors, i.e. $$\text{CRT}(\mathbf{x}) : \mathcal{R}_q^m \to (\mathbb{F}_{q^e})^{md/e}$$ is defined as the concatenation of $$\text{CRT}$$ applied to each entry of the vector. 
Furthermore, we define $$\text{CRT}$$ also for polynomials, i.e. $$\text{CRT} : \mathcal{R}_q^{\nu}[x^{\mu}] \to \mathbb{F}_{q^e}^{d/e}[x^{\mu}]$$ applies $$\text{CRT}$$ to each coefficient vector of the polynomial.
...
![alt text](/post-src/salsaa/image.png)
...
<img src="/post-src/salsaa/7ea41f99cf1a1fc56ba461d084072488.jpg" style="width:60%">

The protocol:
![alt text](/post-src/salsaa/14bb54fc8e9623f46937beec7db85acd.jpg)
